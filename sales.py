...
# --- Sales Tracking Page ---
    ...
    if uploaded_file:
        ...
        if sales_df is not None and target_df is not None:
            ...
            # --- Working days (exclude Fridays=4) in selected range ---
            ...
            # --- Working days in current month (exclude Fridays) for forecast ---
            current_month_start = today.replace(day=1)
            current_month_end = current_month_start + pd.offsets.MonthEnd(0)
            current_month_days = pd.date_range(current_month_start, current_month_end, freq="D")
            working_days_current_month = int(sum(1 for d in current_month_days if d.weekday() != 4))

            # --- Aggregations for KA and Talabat ---
            ...
            all_salesmen_idx = (
                total_sales.index.union(talabat_sales.index)
                .union(ka_targets.index)
                .union(talabat_targets.index)
            )
            ...
            with tabs[0]:
                ...
                if not total_sales_top.empty:
                    r2c1.metric("Top KA Salesman", total_sales_top.idxmax(), f"{total_sales_top.max():,.0f}")
                else:
                    r2c1.metric("Top KA Salesman", "—", "0")
                if not talabat_sales_top.empty:
                    r2c2.metric("Top Talabat Salesman", talabat_sales_top.idxmax(), f"{talabat_sales_top.max():,.0f}")
                else:
                ...
                # Forecast month-end KA = current per-day × working days in current month (Fridays excluded)
            ...
            with tabs[1]:
                ...
                # Build raw wide table by Billing Type
                billing_wide = (
                    df_filtered.groupby(["Driver Name EN", "Billing Type"])["Net Value"]
                    .sum()
                    .unstack(fill_value=0)
                )

                # Ensure required raw SAP billing types exist
                required_cols_raw = ["ZFR", "YKF2", "YKRE", "YKS1", "YKS2", "ZCAN", "ZRE"]
                for col in required_cols_raw:
                    if col not in billing_wide.columns:
                        billing_wide[col] = 0

                # Rename for display and compute fields
                # ZFR -> Sales Group (previously "PRESALES"), YKF2 -> HHT
                display_df = billing_wide.copy()
                display_df.rename(columns={"ZFR": "Sales Group", "YKF2": "HHT"}, inplace=True)

                # Sales Total = sum of all billing columns (use original raw names to be safe)
                display_df["Sales Total"] = (
                    billing_wide[required_cols_raw].sum(axis=1)
                    if set(required_cols_raw).issubset(billing_wide.columns)
                    else billing_wide.sum(axis=1)
                )
                ...
                display_df["Return %"] = np.where(
                    ...
                    (display_df["Return"] / display_df["Sales Total"] * 100).round(2),
                    0
                )

                # Cancel Total = YKS1 + YKS2 + ZCAN
                display_df["Cancel Total"] = (
                    billing_wide["YKS1"] + billing_wide["YKS2"] + billing_wide["ZCAN"]
                )

                # Reorder columns in grouped sections
                ordered_cols = [
                    # Sales Group
                    "Sales Group", "HHT", "Sales Total",
                    # Cancel Group
                    "YKS1", "YKS2", "ZCAN", "Cancel Total",
                    # Returns Group
                    "YKRE", "ZRE", "Return", "Return %"
                ]
                # Make sure all are present even if zeros
                for oc in ordered_cols:
                    if oc not in display_df.columns:
                        display_df[oc] = 0
                ...
                # Recompute Total row's Return % correctly
                if total_row.at["Total", "Sales Total"] != 0:
                    total_row.at["Total", "Return %"] = round(
                        (float(total_row.at["Total", "Return"]) / float(total_row.at["Total", "Sales Total"])) * 100, 2
                    )
                else:
                    total_row.at["Total", "Return %"] = 0
                billing_df = pd.concat([display_df, total_row])

                # Styler: group colors
                sales_group_cols = ["Sales Group", "HHT", "Sales Total"]
                cancel_group_cols = ["YKS1", "YKS2", "ZCAN", "Cancel Total"]
                returns_group_cols = ["YKRE", "ZRE", "Return", "Return %"]

                col_to_color = {}
                for c in sales_group_cols:
                    col_to_color[c] = "background-color: #ccffcc"   # light green
                for c in cancel_group_cols:
                    col_to_color[c] = "background-color: #ffd9b3"   # light orange
                for c in returns_group_cols:
                    col_to_color[c] = "background-color: #fff2b3"   # light yellow
                ...
                st.dataframe(
                    py_df.to_frame().style.background_gradient(cmap="Greens").format("{:,.0f}"),
                    use_container_width=True
                )
            ...
            # =================== Advanced Insights =================
            with tabs[4]:
                st.subheader("Sales Trend Forecast (Simple Linear)")
...
# --- YTD Page ---
    ...
    if uploaded_file:
        ...
        if sales_df is not None:
            ...
            df1 = sales_df[
                (sales_df["Billing Date"] >= pd.to_datetime(period1[0]))
                & (sales_df["Billing Date"] <= pd.to_datetime(period1[1]))
            ]
            df2 = sales_df[
                (sales_df["Billing Date"] >= pd.to_datetime(period2[0]))
                & (sales_df["Billing Date"] <= pd.to_datetime(period2[1]))
            ]
            ...
            def highlight_date_columns(row):
                return [
                    ...
                    if col in [period1_label, period2_label]
                    else ""
...
# --- Custom Analysis (NEW PAGE) ---
    ...
    if uploaded_file:
        ...
        if sales_df is not None:
            ...
            # Derived time columns for convenience
